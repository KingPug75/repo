...
def choose_quality(link, name=None, selected_link=None):
    ...
    if link.startswith("http") or link.startswith("plugin") or link.startswith("magnet:"):
        sublinks = [link]
    else:
        ...
        if "search" in preset:
            ...
            if item.get("content", "") == "song":
                ...
                played = Sources.get_sources(
                    ...
                    exclude=exclude_scrapers,
                    player=jenplayer)
        elif preset.startswith("http") or preset.startswith("plugin") or preset.startswith("magnet:"):
            # direct link
